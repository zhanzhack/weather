...
const translations = {
    PL: {
        ...
        offline: "⚠️ Tryb Offline (Dane z pamięci)", online: "Z powrotem online"
    },
    EN: {
        ...
        offline: "⚠️ Offline Mode (Cached data)", online: "Back online"
    }
};
...
function renderChart(temps) {
    ...
    myChart = new Chart(ctx, {
        ...
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { display: false }, x: { ticks: { color: '#a0aec0' } } } }
    });
}
...
function getWeatherDesc(code) {
    const desc = { PL: { 0: "Czyste niebo", 3: "Pochmurno", 61: "Deszcz" }, EN: { 0: "Clear sky", 3: "Cloudy", 61: "Rain" } };
    return desc[currentLang][code] || (currentLang === 'PL' ? "Pochmurno" : "Cloudy");
}

function getWeatherEmoji(code) { return code === 0 ? "☀️" : code < 4 ? "⛅" : "🌧️"; }

// --- ПРИВ'ЯЗКА ПОДІЙ (SHARE FIX) ---
document.addEventListener('DOMContentLoaded', () => {
    document.getElementById('share-btn').onclick = async () => {
        ...
        try {
            await navigator.share({
                title: 'SkyCast Pro',
                text: `Pogoda w ${weatherData.cityName}: ${convertTemp(weatherData.current.temperature_2m)}°C`,
    ...
    document.getElementById('search-btn').onclick = async () => {
        ...
        if(data.results) {
            getWeatherData(data.results[0].latitude, data.results[0].longitude, data.results[0].name);
            showPage('home');
        }
    };
...
window.addEventListener('offline', () => {
    document.getElementById('weather-desc').innerText = translations[currentLang].offline;
    document.body.style.filter = "grayscale(0.4)";
});

window.addEventListener('online', () => {
    document.getElementById('weather-desc').innerText = translations[currentLang].online;
